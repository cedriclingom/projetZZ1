...
void RechercheSommet(int * x, nombre_t ** sommet, int NbSommet, enum bool * ptrouver)
{
  ...
  *ptrouver = faux;
  ...
  while((i < (NbSommet-1)) && (sommet[i][0].entier != *x))
    {
...
void RechercheArrete(int a, nombre_t ** arrete, int NbArretes, enum bool *ptrouver, int * x, int * y)
{
  ...
  *ptrouver = faux;
  ...
  while ((i < NbArretes) && (arrete[i][0].entier != a))
    {
...
void RemplirMatriceDistancePhase1(nombre_t ** distance , nombre_t ** sommet, nombre_t ** arrete, int NbSommet, int NbArrete)
{
  ...
  for (i = 0; i < NbSommet; ++i)
      ...
      for (j = i + 1; j < NbSommet; ++j)
	{
	  ...
	  RechercheCouple(sommet[i][0].entier, sommet[j][0].entier, arrete, NbArrete, &trouver);
	  
	  if(trouver)
	    {
	      
	      distance[i][j].reel = CalculDistance(sommet[i][1].entier, sommet[i][2].entier, sommet[j][1].entier, sommet[j][2].entier);
	      
	      distance[j][i].reel = distance[i][j].reel;
	  ...
	  if((trouver1) && (trouver2))
	    {
	      ...
	      distance[y][i].reel = distance[i][y].reel;
	  ...
	  for(j = 0; j < (NbSommet + NbPointArret); ++j)
	    {
	      ...
	      if(distance[i][j].reel > (distance[i][k].reel + distance[k][j].reel))
		{
		  
		  distance[i][j].reel = distance[i][k].reel + distance[k][j].reel;
	  ...
	  for(j = i + 1; j < dimension; ++j)
	    {
	      ...
	      DistancePA[i - NbSommet][j - NbSommet].reel = distance[i][j].reel;

	      DistancePA[j - NbSommet][i - NbSommet].reel = DistancePA[i - NbSommet][j - NbSommet].reel;
